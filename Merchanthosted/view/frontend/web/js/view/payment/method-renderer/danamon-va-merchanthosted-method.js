...
define(
        ...
        function (Component, $, url, alert, checkout, loader) {
            ...
            return Component.extend({
                defaults: {
                    template: 'Doku_Merchanthosted/payment/danamon-va-merchanthosted',
                    setWindow: false,
                    dokuObj: {},
                    dokuDiv: ''
                },
                        ...
                        success: function (response) {
                            ...
                            if (dataResponse.err == false) {
                                jQuery.each(dataResponse.result, function (i, val) {
                                    if (i != 'URL') {
                                        $("#danamon-va-merchanthosted").append('<input type="hidden" name="' + i + '" value="' + val + '">');
                                    } else {
                                        $("#danamon-va-merchanthosted").attr("action", val);
                                    }
                                    $("#danamon-va-merchanthosted").submit();
                                });
                            } else {
                                alert({
                                    title: 'Payment error!',
...
//                window.location = url.build('dokuhosted/payment/request');
                ...
                getDescription: function(){
                     return window.checkoutConfig.payment.danamon_va_merchanthosted.description
                }
            });
        }
);

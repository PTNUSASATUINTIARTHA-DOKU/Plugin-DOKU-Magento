...
class RequestMerchanthosted extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        $this->logger->info('===== Request controller  (merchanthosted) ===== Start');

        $this->logger->info('===== Request controller  (merchanthosted) ===== Find Order');
        ...
        if ($order->getEntityId()) {
            ...
            $this->logger->info('===== Request controller  (merchanthosted) ===== Order Found!');
            ...
            $doku_js_url = $this->assetRepo->getUrl("Doku_Merchanthosted::js/doku/doku-1.3.2.js", $isSecureParams);
...
//            $doku_js_url = $this->generalConfiguration->getJsMechantHosted();
            ...
            $params = array(
                ...
                'req_words' => $words,
                'req_form_type' => 'full',
                'req_server_url' =>  $base_url."dokumerchanthosted/payment/chargemerchanthosted",
                'req_session_id' => $order->getIncrementId(),
                'doku_js_url' => $doku_js_url
            );

            $this->logger->info('===== Request controller  (merchanthosted) ===== request payment = ' . json_encode($params, JSON_PRETTY_PRINT));
            ...
            $this->logger->info('===== Request controller  (merchanthosted) ===== Order not found');
            
            echo json_encode(array('err' => true, 'response_msg' => 'Generate request failed (order not found}',
                'result' => array()));
        }

        $this->logger->info('===== Request controller  (merchanthosted) ===== end');
    }

...
define(
        ...
        function (Component, $, url, alert, checkout, loader) {
            ...
            return Component.extend({
                defaults: {
                    template: 'Doku_Hosted/payment/ib-permata-hosted',
                    setWindow: false,
                    dokuObj: {},
                    dokuDiv: ''
                },
                        ...
                        success: function (response) {
                            ...
                            if (dataResponse.err == false) {
                                jQuery.each(dataResponse.result, function (i, val) {
                                    if (i != 'URL') {
                                        $("#ib-permata-hosted").append('<input type="hidden" name="' + i + '" value="' + val + '">');
                                    } else {
                                        $("#ib-permata-hosted").attr("action", val);
                                    }
                                });
                                $("#ib-permata-hosted").submit();
                            } else {
                                alert({
                                    title: 'Payment error!',
...
//                window.location = url.build('dokuhosted/payment/request');
                ...
                getDescription: function(){
                     return window.checkoutConfig.payment.ib_permata_hosted.description
                }
            });
        }
);

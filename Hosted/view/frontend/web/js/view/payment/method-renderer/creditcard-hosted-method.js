...
define(
        ...
        function (Component, $, url, alert, checkout, loader, totals) {
            ...
            return Component.extend({
                defaults: {
                    template: 'Doku_Hosted/payment/creditcard-hosted',
                    setWindow: false,
                    dokuObj: {},
                    dokuDiv: ''
                },
                        ...
                        success: function (response) {
                            ...
                            if (dataResponse.err == false) {
                                jQuery.each(dataResponse.result, function (i, val) {
                                    if (i != 'URL') {
                                        $("#credit-card-hosted").append('<input type="hidden" name="' + i + '" value="' + val + '">');
                                    } else {
                                        $("#credit-card-hosted").attr("action", val);
                                    }
                                });
                                $("#credit-card-hosted").submit();
                            } else {
                                alert({
                                    title: 'Payment error!',
...
//                window.location = url.build('dokuhosted/payment/request');
                ...
                getDescription: function(){
                     return window.checkoutConfig.payment.cc_hosted.description
                },

...
define(
        ...
        function (Component, $, url, alert, checkout, loader) {
            ...
            return Component.extend({
                defaults: {
                    template: 'Doku_Hosted/payment/mandiri-va-hosted',
                    setWindow: false,
                    dokuObj: {},
                    dokuDiv: ''
                },
                        ...
                        success: function (response) {
                            ...
                            if (dataResponse.err == false) {
                                jQuery.each(dataResponse.result, function (i, val) {
                                    if (i != 'URL') {
                                        $("#mandiri-va-hosted").append('<input type="hidden" name="' + i + '" value="' + val + '">');
                                    } else {
                                        $("#mandiri-va-hosted").attr("action", val);
                                    }
                                });
                                $("#mandiri-va-hosted").submit();
                            } else {
                                alert({
                                    title: 'Payment error!',
...
//                window.location = url.build('dokuhosted/payment/request');
                ...
                getDescription: function(){
                     return window.checkoutConfig.payment.mandiri_va_hosted.description
                }
            });
        }
);

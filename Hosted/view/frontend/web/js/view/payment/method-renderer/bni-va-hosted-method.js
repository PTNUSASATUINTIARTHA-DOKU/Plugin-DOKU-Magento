...
define(
        ...
        function (Component, $, url, alert, checkout, loader) {
            ...
            return Component.extend({
                defaults: {
                    template: 'Doku_Hosted/payment/bni-va-hosted',
                    setWindow: false,
                    dokuObj: {},
                    dokuDiv: ''
                },
                        ...
                        success: function (response) {
                            ...
                            if (dataResponse.err == false) {
                                jQuery.each(dataResponse.result, function (i, val) {
                                    if (i != 'URL') {
                                        $("#bni-va-hosted").append('<input type="hidden" name="' + i + '" value="' + val + '">');
                                    } else {
                                        $("#bni-va-hosted").attr("action", val);
                                    }
                                });
                                $("#bni-va-hosted").submit();
                            } else {
                                alert({
                                    title: 'Payment error!',
...
//                window.location = url.build('dokuhosted/payment/request');
                ...
                getDescription: function(){
                     return window.checkoutConfig.payment.bni_va_hosted.description
                }
            });
        }
);
